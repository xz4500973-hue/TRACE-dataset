/* PID testbench - 64bit bus, high frequency, dynamic PV sweep
 */
module PID_tb;
reg clk=0; reg rst=1;
...
initial begin
  $dumpfile("PID_tb.vcd");
	$dumpvars(0, PID_tb);
  #200;
  wb_write(0, 64'd1000);  // kp=1000
  wb_write(4, 64'd50);    // ki=50
  wb_write(8, 64'd400);   // kd=400
  wb_write(12, 64'd2000); // sp=2000
  wb_write(44, 64'd0);    // RS=0
  // Dynamic PV sweep with large values
  for(i=0; i<10; i=i+1) begin
    wb_write(16, i*200 + 100); repeat(200) @(posedge clk);
  end

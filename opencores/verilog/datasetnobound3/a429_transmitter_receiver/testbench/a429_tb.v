...
module a429_tb_top;
    ...
    initial begin
        ...
        #1000 enable = 1;
        seed = 77777;
        // ---------- Test mode 1: 100Kbps, max gap ----------
        $display("Mode 1: 100Kbps, maximum gap");
        speed = 2'b00;
        gap_bits = 7'd64;
        num_words = 80;
        for (i=0; i<num_words; i=i+1) begin
            wait(tx_ready);
            tx_data = {$random(seed+3000)}[31:0];
            parcheck = 1'b1;
            @(posedge clk2M);
            tx_req = 1'b1;
            @(posedge clk2M);
        ...
        // ---------- Test mode 2: 12.5Kbps, all-ones stress ----------
        $display("Mode 2: 12.5Kbps, all-ones pattern");
        speed = 2'b10;
        gap_bits = 7'd32;
        num_words = 300;
        for (i=0; i<num_words; i=i+1) begin
            wait(tx_ready);
            tx_data = 32'hFFFF_FFFF;
            parcheck = 1'b0;
            @(posedge clk2M);
            tx_req = 1'b1;
            @(posedge clk2M);

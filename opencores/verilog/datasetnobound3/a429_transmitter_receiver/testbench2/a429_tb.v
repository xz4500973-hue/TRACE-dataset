...
module a429_tb_top;
    ...
    initial begin
        ...
        // ---------- Test mode 1: Mix of short & long bursts ----------
        $display("Mode 1: short bursts at 100Kbps");
        speed = 2'b00;
        gap_bits = 7'd10;
        num_words = 150;
        for (i=0; i<num_words; i=i+1) begin
            wait(tx_ready);
            tx_data = {$random(seed+8000)}[31:0];
            parcheck = (i < 75) ? 1'b1 : 1'b0;
            @(posedge clk2M);
            tx_req = 1'b1;
            @(posedge clk2M);
            tx_req = 1'b0;
        ...
        // ---------- Test mode 2: 12.5Kbps with varying gaps ----------
        $display("Mode 2: 12.5Kbps, varying gaps");
        speed = 2'b10;
        gap_bits = 7'd20;
        num_words = 120;
        for (i=0; i<num_words; i=i+1) begin
            wait(tx_ready);
            tx_data = {i[15:0], i[15:0]};
            parcheck = 1'b1;
            @(posedge clk2M);
            tx_req = 1'b1;
            @(posedge clk2M);

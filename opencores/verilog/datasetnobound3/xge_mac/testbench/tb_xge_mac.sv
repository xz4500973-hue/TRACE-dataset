...
// Clock: different phase
initial begin
    clk_156m25 = 1'b1; clk_xgmii_rx = 1'b1; clk_xgmii_tx = 1'b1;
    forever begin #3200; clk_156m25 = ~clk_156m25; clk_xgmii_rx = ~clk_xgmii_rx; clk_xgmii_tx = ~clk_xgmii_tx; end
end
initial begin wb_clk_i = 1'b1; forever begin #20000; wb_clk_i = ~wb_clk_i; end end

// Staggered reset sequence
initial begin
    reset_156m25_n = 1'b0; reset_xgmii_rx_n = 1'b0; reset_xgmii_tx_n = 1'b0;
    #15000; reset_156m25_n = 1'b1;
    #10000; reset_xgmii_tx_n = 1'b1;
    #10000; reset_xgmii_rx_n = 1'b1;
end
...
initial begin
    for (tx_length = 0; tx_length <= 3000; tx_length = tx_length + 1) tx_buffer[tx_length] = 0;
    pkt_rx_ren = 1'b0; pkt_tx_data = 64'b0; pkt_tx_val = 1'b0;
    pkt_tx_sop = 1'b0; pkt_tx_eop = 1'b0; pkt_tx_mod = 3'b0;
end

// Send single long packet
task SendLongPacket;
  input [31:0] len;
  input [7:0] seed;
  integer i;
  begin
    tx_length = len;
    for (i = 0; i < len; i = i + 1)
        tx_buffer[i] = seed + (i % 16);
    @(posedge clk_156m25); #1500;
    pkt_tx_val = 1'b1;
...
initial begin
    #3000000;
    // Send 2 long packets and 2 medium ones
    SendLongPacket(512, 8'h10);
    #3000000;
    SendLongPacket(256, 8'hf0);
    #3000000;
    SendLongPacket(128, 8'h88);
    #3000000;
    SendLongPacket(800, 8'h3c);
    #10000000;
    $finish;
end

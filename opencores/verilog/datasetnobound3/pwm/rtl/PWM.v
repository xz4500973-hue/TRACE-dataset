...
always@(posedge i_wb_clk or posedge i_wb_rst)
	if(i_wb_rst)begin
		...
		if(i_wb_adr == adr_ctrl) begin
			ctrl[4:0]<=i_wb_data[4:0];
			ctrl[7:6]<=i_wb_data[7:6];
		end
		else if(i_wb_adr == adr_divisor)
			divisor<=i_wb_data;
		else if(i_wb_adr == adr_period)
			period<=i_wb_data;
		else if(i_wb_adr == adr_DC)
			DC<=i_wb_data;
	end
...
wire	intt;		//signal from pwm/timer  logic
always@(posedge i_wb_clk or posedge i_wb_rst)
	if(i_wb_rst)begin
		ctrl[5]<=0;
		state<=0;

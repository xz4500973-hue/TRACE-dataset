...
reg f_edge, r_edge;
always @(*) begin
	f_edge = e_bit[1] & (~e_bit[0]);
	r_edge = (~e_bit[1]) & e_bit[0];
end
...
reg [19:0] count_next_reg;
always @(*) begin
	if(c_load)
		count_next_reg = count_reg + 1'b1;
	else
		count_next_reg = 20'h00000;
end
...
begin
	...
	if(state_reg == IDLE) begin
		if(f_edge)
			state_next = START;
	end else if(state_reg == START) begin
		c_load = 1'b1;
		if(r_edge) begin
			if(count_reg > START_TIME) begin
				state_next = SYNC;
				c_load = 1'b0;
			end else begin
				state_next = IDLE;
	...
	end else if(state_reg == SYNC) begin
		c_load = 1'b1;
		if(f_edge) begin
			if(count_reg > SYNC_TIME) begin
	...
	end else if(state_reg == WAIT_HIGH) begin
		if(r_edge)
			state_next = FETCH;
	end else if(state_reg == WAIT_LOW) begin
		if(f_edge)
			state_next = WAIT_HIGH;
	end else if(state_reg == FETCH) begin
		c_load = 1'b1;
		if(count_reg > CENTER) begin
			c_bit_next = led_reg[0];
			led_next = {ir_in,led_reg[31:1]};
			c_load = 1'b0;
			state_next = BCOUNT;
		end
	end else if(state_reg == BCOUNT) begin
		if(c_bit_reg)
			state_next = DONE;
		else begin
			if(led_reg[31])
		...
		if(((led_reg[7:0]^led_reg[15:8]) == 8'hff)&&((led_reg[23:16]^led_reg[31:24]) == 8'hff)) begin
			done_tick = 1'b1;

...
/* refactored: 5 always blocks reorganized into 3 always blocks */
(* keep_hierarchy = "yes" *) module point_scalar_mult(clk, reset, x1, y1, zero1, c, done, x3, y3, zero3);
    ...
    // Always block 1: k counter
    always @ (posedge clk)
        if (reset) k <= c;
        else if (op & p) k <= k >> 1;

    // Always block 2: op toggle + x4 accumulator
    always @ (posedge clk) begin
        if (reset) begin
            op <= 0;
            {x4,y4,zero4} <= {x1,y1,zero1};
        end else begin
            if (p) op <= ~op;
            if (op & p) begin {x4,y4,zero4} <= {x7,y7,zero7}; end
        end
    end

    // Always block 3: x2 accumulator + output register
    always @ (posedge clk) begin
        if (reset) begin
            x2 <= 0; y2 <= 0; zero2 <= 1;

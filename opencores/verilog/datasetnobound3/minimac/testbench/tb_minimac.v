...
// Minimal data path activity (no packets)
...
// Generate continuous CSR writes to different addresses
integer csr_loop;
initial begin
	csr_loop = 0;
end
always @(posedge sys_clk) begin
	if(~sys_rst & csr_loop < 10000) begin
		// Write to random CSR address (within valid range)
		csrwrite( ({$random} % 16) * 4, {$random} );
		csr_loop <= csr_loop + 1;
	end
end

initial begin
	$dumpfile("minimac_csr_stress.vcd");
	$dumpvars(0, tb_minimac);
	sys_rst = 1'b1;
	waitclock;
	sys_rst = 1'b0;

	// Enable basic functions (e.g., RX and TX)

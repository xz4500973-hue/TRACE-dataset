/*
 * Mixed workload: moderate TX/RX + CSR access
 */
module tb_minimac();
...
// Back-end: ack occasionally with random latency
always @(posedge sys_clk) begin
	if(wbrx_cyc_o & wbrx_stb_o & ~wbrx_ack_i & (($random % 4) == 0)) begin
		wbrx_ack_i <= 1'b1;
	end else
		wbrx_ack_i <= 1'b0;
	if(wbtx_cyc_o & wbtx_stb_o & ~wbtx_ack_i & (($random % 4) == 0)) begin
		wbtx_dat_i = $random;
		wbtx_ack_i <= 1'b1;
	end else
		wbtx_ack_i <= 1'b0;
...
// PHY RX: occasional packets (1 every ~400 cycles)
reg [7:0] pkt_len;
reg [3:0] pkt_cnt;
initial begin
	phy_dv = 1'b0;
...
// TX software: occasional packets (total 50)
reg tx_trigger;
integer tx_count;
initial begin
	tx_trigger = 0;
...
// Occasional CSR read/write during operation
integer csr_access;
initial begin
	csr_access = 0;
end
always @(posedge sys_clk) begin
	if(~sys_rst & (csr_access < 200)) begin
		if($random % 1000 == 0) begin
			csrread( ({$random} % 16) * 4 );
			csr_access <= csr_access + 1;
		end
	end
end

initial begin
	$dumpfile("minimac_mixed.vcd");

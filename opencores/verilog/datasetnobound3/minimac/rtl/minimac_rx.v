...
(* keep_hierarchy = "yes" *) module minimac_rx(
	input sys_clk,
	input sys_rst,
	input rx_rst,
...
always @(posedge sys_clk) begin
	if(sys_rst|rx_rst)
		...
		if(loadbyte_en) begin
			if(loadbyte_counter == 2'd0)
				wbm_dat_o[31:24] <= fifo_data;
			else if(loadbyte_counter == 2'd1)
				wbm_dat_o[23:16] <= fifo_data;
			else if(loadbyte_counter == 2'd2)
				wbm_dat_o[15: 8] <= fifo_data;
			else
				wbm_dat_o[ 7: 0] <= fifo_data;
		end
	end
end

...
module tb ();
  ...
  integer i,j;
  reg signed [16:0] z [0:30];

  initial begin
    $dumpfile("cordic.vcd");
    $dumpvars(0, tb);

    // Only first quadrant, half resolution
    for(j=0;j<=30;j=j+1) z[j] = j * 1715; // ~3 degree steps
    ...
    for(i=0;i<(`ITERATIONS+5);i=i+1) begin #1 clock<=1;#1 clock<=0; end

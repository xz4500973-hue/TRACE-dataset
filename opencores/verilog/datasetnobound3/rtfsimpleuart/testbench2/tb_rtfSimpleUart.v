...
module tb_rtfSimpleUart;
    ...
    initial clk=0; always #3 clk=~clk;  // ~167MHz
    ...
    task uart_send_byte; input [7:0] byter; integer i;
        begin rxd=1'b0; repeat(BIT_CLKS) @(posedge clk);
        for(i=0;i<8;i=i+1) begin rxd=byter[i]; repeat(BIT_CLKS) @(posedge clk); end
    ...
    initial begin
        ...
        uart_send_byte(8'hCC); while(!data_present) @(posedge clk);
        uart_send_byte(8'h33); while(!data_present) @(posedge clk);
        uart_send_byte(8'h7E); while(!data_present) @(posedge clk);
        uart_send_byte(8'h81); while(!data_present) @(posedge clk);
        uart_send_byte(8'h99); while(!data_present) @(posedge clk);
        #60000;

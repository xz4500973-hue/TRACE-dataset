...
module tb_sip_rxaui;
    ...
    integer i;
    reg [31:0] seed_tx, seed_rx;

    initial begin
        $dumpfile("rxaui_full_duplex.vcd");
        ...
        fork
            ...
            begin
                for (i = 0; i < 300; i = i + 1) begin
                    @(posedge s_tx_clk);
                    txdata_serdes0 = {$random(seed_tx)} % 1048576;
                    txdata_serdes1 = {$random(seed_tx)} % 1048576;
                end
            end
            // 接收线程：300 个随机数据，每隔 10 周期插入逗号
            begin
                for (i = 0; i < 300; i = i + 1) begin
                    @(posedge s_rx_clk);
                    if (i % 10 == 0)
                        ...
                        rxaui_rx_data = {$random(seed_rx)} % 1048576;
                end
            end
        join

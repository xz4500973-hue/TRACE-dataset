...
module simReedSolomon;
   ...
   //------------------------------------------------------------------------
    ReedSolomon_Top u_dut (
        .CLK            (CLK),
        .RESET          (RESET),
        // 编码器
        .enc_enable     (enc_enable),
        .enc_startPls   (enc_startPls),
        .enc_dataIn     (enc_dataIn),
        .enc_dataOut    (enc_dataOut),
        // 解码器
        .dec_enable     (dec_enable),
        .dec_sync       (dec_sync),
        .dec_erasureIn  (dec_erasureIn),
        .dec_dataIn     (dec_dataIn),
        .dec_outStartPls(dec_outStartPls),
        .dec_outDone    (dec_outDone),
        .dec_outData    (dec_outData),
        .dec_errorNum   (dec_errorNum),
        .dec_erasureNum (dec_erasureNum),
        .dec_fail       (dec_fail),
        .dec_outEnable  (dec_outEnable),
        .dec_delayedData(dec_delayedData)
    );
...
//--------------------------------------------------------------------------
   ...
   always @(posedge CLK or negedge RESET) begin
      ...
      else if ((rsencOBankIndex > 32'd383) && (rsdecOBankIndex > 32'd1200)) begin
         simOver <= 1'b1;
         $fclose(handleA);
         $finish;
      end
   ...
   //------------------------------------------------------------------------
   initial begin
      $dumpfile("reed_solomon_gate.vcd");
      $dumpvars(0, simReedSolomon);
      simStart = 1'b0;
      CLK = 0;
      RESET = 1;
      #(period*2)	RESET = 0;

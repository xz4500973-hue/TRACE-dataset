...
module tb_user_logic;
    ...
    always @(posedge clk) begin
        if(rst) begin
            DQ_in<=1'b1; sl_state<=0; sl_timer<=0; sl_byte<=8'hFF; sl_bcnt<=0;
        end else begin
            case(sl_state)
                0: if(!DQ_out && !DQ_oe) begin sl_state<=1; sl_timer<=0; end
                1: if(DQ_oe) begin
                    if(sl_timer<30) begin sl_timer<=sl_timer+1; DQ_in<=1'b1; end
                    else begin sl_state<=2; sl_timer<=0; DQ_in<=1'b0; end
                end
                2: begin
                    if(sl_timer<120) begin sl_timer<=sl_timer+1; DQ_in<=1'b0; end
                    else begin DQ_in<=1'b1; sl_state<=0; end
                end
            endcase
            if(sl_state==0 && !DQ_out && !DQ_oe) begin
                if(DQ_oe) begin DQ_in<=sl_byte[0]; sl_byte<={1'b0,sl_byte[7:1]}; end
            end
        end
    end
    ...
    initial begin
        ...
        #100; rst=0; #100;
        // 10 alternating write/read operations
        for(i=0; i<20; i=i+1) begin
            $display("Bus reset %0d", i); start_operation(3'd3, 8'd0);
            $display("Write 0x%0h", i*16+i); start_operation(3'd1, i*14+i);

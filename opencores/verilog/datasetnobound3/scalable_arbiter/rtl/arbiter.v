...
(* keep_hierarchy = "yes" *) module mux_array #(
	parameter width = 0
)(
...
(* keep_hierarchy = "yes" *) module mux_node (
	input [1:0] in,
	input select,
	output out
);
...
(* keep_hierarchy = "yes" *) module arbiter_node (
	input enable,
	input [1:0] req,
	output [1:0] grant,
	output select,
...
*/
// Dataset3: pipeline next_state (extra flop)
reg next_state_pipe;
always @(req or grant_state) begin
    next_state_pipe = (req[1] & ~req[0]) | (req[1] & grant_state);
end
// Extra pipeline stage
reg next_state;
always @(posedge clock or posedge reset) begin
    if (reset) next_state <= 0;
    else       next_state <= next_state_pipe;
end

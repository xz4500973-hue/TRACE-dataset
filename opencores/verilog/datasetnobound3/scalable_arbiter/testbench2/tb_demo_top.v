/*
 * Testbench for demo_top - Long press (stretch test)
 */
...
module tb_demo_top;
    ...
    initial begin
        $dumpfile("demo_top_long_press.vcd");
        $dumpvars(0, u_dut);
        $display("=== Starting demo_top testbench (long press) ===");

        buttons = 2'b00;
        #50000;

        // Long press button 1 (next_test) for 2 ms (stretch should activate)
        press_button(1, 2000);

        #2000000;

        $display("=== Long press testbench finished ===");
        $finish;
    end

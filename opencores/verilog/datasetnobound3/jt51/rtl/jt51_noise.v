...
(* keep_hierarchy = "yes" *) module jt51_noise(
	input	rst,
	input	clk,
	input	zero,
	input	ne,
...
always @(posedge clk) 
	if( ne ) begin
		if( zero_out )
			// priority encoder style (was casex)
				if(~eg[9]) limit <= 10'h3FF;
				else if(~eg[8]) limit <= 10'h1FF;
				else if(~eg[7]) limit <= 10'h0FF;
				else if(~eg[6]) limit <= 10'h07F;
				else if(~eg[5]) limit <= 10'h03F;
				else if(~eg[4]) limit <= 10'h01F;
				else if(~eg[3]) limit <= 10'h00F;
				else if(~eg[2]) limit <= 10'h007;
				else if(~eg[1]) limit <= 10'h003;
				else limit <= 10'h001;		
	end
	else limit <= 10'd0;

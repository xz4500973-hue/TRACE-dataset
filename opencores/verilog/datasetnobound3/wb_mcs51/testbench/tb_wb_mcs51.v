...
module tb_wb_mcs51;
    ...
    always @(posedge clk_i or negedge nrst_i)
        if(!nrst_i) begin wbm_ack_i<=0;wbm_dat_i_reg<=0;wbm_rty_i<=0;wbm_err_i<=0; end
            ...
            if(wbm_cyc_o&&wbm_stb_o&&!wbm_ack_i) begin wbm_ack_i<=1;if(!wbm_we_o) wbm_dat_i_reg<=$urandom%256; end
    ...
    initial begin
        ...
        // Pattern: AA 0x64 locations then readback
        for(i=0;i<100;i=i+1) begin wr(i[7:0],8'hAA); end
        for(i=0;i<100;i=i+1) begin rd(i[7:0],r); end

        // Pattern: 55 alternating
        for(i=0;i<100;i=i+1) begin wr(i[7:0],8'h55); end
        for(i=0;i<100;i=i+1) begin rd(i[7:0],r); end

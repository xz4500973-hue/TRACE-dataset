...
module a429_tb_top;
    ...
    initial begin
        ...
        seed = 12345;
        // ---------- Test mode 1: 100Kbps, random data ----------
        $display("Mode 1: 100Kbps, random data, variable parity check");
        speed = 2'b00;   // 100Kbps (speed[0]=0)
        gap_bits = 7'd8;
        num_words = 200;
        for (i=0; i<num_words; i=i+1) begin
            wait(tx_ready);
            tx_data = {$random(seed)}[31:0];
            parcheck = ($random(seed) % 2) ? 1'b1 : 1'b0;   // toggle check mode
            @(posedge clk2M);
            tx_req = 1'b1;
            @(posedge clk2M);
            tx_req = 1'b0;
        ...
        // ---------- Test mode 2: 12.5Kbps, long gap, alternating data ----------
        $display("Mode 2: 12.5Kbps, long gap, alternating pattern");
        speed = 2'b10;     // speed[0]=1 → 12.5Kbps
        gap_bits = 7'd60;
        num_words = 100;
        for (i=0; i<num_words; i=i+1) begin
            wait(tx_ready);
            tx_data = (i%2) ? 32'hAAAA_AAAA : 32'h5555_5555;
            parcheck = 1'b1;   // always check parity (pattern has correct odd parity? – just for demo)
            @(posedge clk2M);
            tx_req = 1'b1;
            @(posedge clk2M);
            tx_req = 1'b0;

...
always@(posedge i_clk or negedge i_rst)
	if(!i_rst)begin
		sdata<=0; shifted<=0; shifted_1<=0;
		PR<=0; PR_1<=0; DR<=0;
		ready<=0; ct<=0; ct_1<=0;
		ct_1_en<=0; ct_1_en_1<=0;
		state<=state_1; DD_sign<=0;
		o_quotient<=0; o_remainder<=0;
		nq<=0; q<=0;
		reg_a<=0; reg_b<=0; reg_carry<=0; state_reg<=0;
	end
	else begin
		if(ready&&(!i_start))ready<=0;
		// Dataset2: if-else refactored (replaces nested case)
		if (state_reg == 0) begin
			if (state == state_1) begin
				if ((!ready)&&i_start) begin
					sdata<=i_divisor; state<=state_6;
					q<=0; nq<=0; shifted<=0;
				end
			end
			else if (state == state_2) begin
				sdata<=i_dividend; PR_1<=i_dividend;
				DD_sign<=i_dividend[31]; state<=state_3; state_reg<=1;
			end
			else if (state == state_3) begin
				if (ct[5]) begin
					state<=state_4; ct<=0; state_reg<=2;
				end else begin
					ct_1_en<=1; shifted_1<=shifted;
					nq[ct]<=nsub; q[ct]<=~nsub;
					sdata<=sum; state_reg<=1;
				end
			end
			else if (state == state_4) begin
				state<=state_5; o_quotient<=sum;
				PR_1<=PR_1>>>ct_1; state_reg<=2;
			end
			else if (state == state_5) begin
				o_remainder<=sum; ct_1<=0;
				state<=state_1; ready<=1;
			end
			else if (state == state_6) begin
				sdata<=sdata_o; shifted<=shifted_o;
				ct<=sum_ct;
				if (sdata[31]!=sdata[30]) begin
					state<=state_2; DR<=sdata;
				end
			end
		end
		else if (state_reg == 1) begin
			PR<=sdata_o; PR_1<=sdata;
			shifted<=shifted_o; state_reg<=2;
			reg_a<=a; reg_b<=b; reg_carry<=carry_in;
			ct_1_en<=0; ct_1_en_1<=ct_1_en;
		end
		else if (state_reg == 2) begin
			if (state==state_3) ct<=sum_ct;
			state_reg<=0;
			if (ct_1_en_1) ct_1<=sum[5:0];
			ct_1_en_1<=0;
			reg_a<=a; reg_b<=b; reg_carry<=carry_in;
		end
	end

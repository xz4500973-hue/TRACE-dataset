...
// Refactored: dataset2 variant - combined RX/TX reset, ternary WB mux
//
// License:	GPL, v3
//
////////////////////////////////////////////////////////////////////////////////
...
(* keep_hierarchy = "yes" *) module	wbuart(i_clk, i_rst,
	...
	// Refactored RX/TX reset: combined into single proc
	reg	tx_uart_reset;
	always @(posedge i_clk) begin
		rx_uart_reset <= (i_rst || ((i_wb_stb)&&(i_wb_addr[1:0]==`UART_SETUP)&&(i_wb_we))) ? 1'b1 :
		                 ((i_wb_stb)&&(i_wb_addr[1:0]==`UART_RXREG)&&(i_wb_we)) ? i_wb_data[12] : 1'b0;
		tx_uart_reset <= (i_rst || ((i_wb_stb)&&(i_wb_addr == `UART_SETUP)&&(i_wb_we))) ? 1'b1 :
		                 ((i_wb_stb)&&(i_wb_addr[1:0]==`UART_TXREG)&&(i_wb_we)) ? i_wb_data[12] : 1'b0;
	end
	...
	// RX errors (unchanged logic, tightened formatting)
	...
	// ===================================================
	// Wishbone response (refactored: pre-compute all data in comb)
	// ===================================================
	reg	[1:0]	r_wb_addr;
	always @(posedge i_clk)
		r_wb_addr <= i_wb_addr;

	// Pre-compute all 4 possible read results combinationally
	wire [31:0] wb_data_setup = { 1'b0, uart_setup };
	wire [31:0] wb_data_next;
	assign wb_data_next = (r_wb_addr == `UART_SETUP) ? wb_data_setup :
	                       (r_wb_addr == `UART_FIFO)  ? wb_fifo_data  :
	                       (r_wb_addr == `UART_RXREG) ? wb_rx_data    :
	                                                    wb_tx_data;

...
module tb_wb_mcs51;
    ...
    wb_mcs51 #(.mcs51_aw(mcs51_aw), .wb_aw(wb_aw)) uut (
        .nrst_i(nrst_i), .clk_i(clk_i), .mcs51_ale(mcs51_ale),
        .mcs51_rd(mcs51_rd), .mcs51_wr(mcs51_wr), .mcs51_ad_inout(mcs51_ad_inout),
        .wbm_adr_o(wbm_adr_o), .wbm_dat_i(wbm_dat_i), .wbm_dat_o(wbm_dat_o),
        .wbm_sel_o(wbm_sel_o), .wbm_cyc_o(wbm_cyc_o), .wbm_stb_o(wbm_stb_o),
        .wbm_we_o(wbm_we_o), .wbm_ack_i(wbm_ack_i), .wbm_rty_i(wbm_rty_i), .wbm_err_i(wbm_err_i));

    always @(posedge clk_i or negedge nrst_i)
        if (!nrst_i) begin wbm_ack_i<=0; wbm_dat_i_reg<=0; wbm_rty_i<=0; wbm_err_i<=0; end
        else begin
            if (wbm_cyc_o && wbm_stb_o && !wbm_ack_i) begin
                wbm_ack_i<=1; if(!wbm_we_o) wbm_dat_i_reg<=$random%256; end
            else wbm_ack_i<=0;
        end
    ...
    task mcs51_write;
        input [mcs51_aw-1:0] addr; input [7:0] data;
        begin
            mcs51_ale=1;mcs51_rd=1;mcs51_wr=1;mcs51_ad_oe=1;mcs51_ad_drive=addr;
            #20;mcs51_ale=0;#10;mcs51_ad_drive=data;mcs51_wr=0;#40;mcs51_wr=1;#10;mcs51_ad_oe=0;
        end
    endtask

    task mcs51_read;
        input [mcs51_aw-1:0] addr; output [7:0] data;
        begin
            mcs51_ale=1;mcs51_rd=1;mcs51_wr=1;mcs51_ad_oe=1;mcs51_ad_drive=addr;
            #20;mcs51_ale=0;#10;mcs51_ad_oe=0;mcs51_rd=0;#40;data=mcs51_ad_inout;mcs51_rd=1;#10;
        end
    endtask

    integer i; reg [7:0] rd;
    ...
    initial begin
        ...
        // Variant: sequential address write
        for(i=0;i<64;i=i+1) begin
            mcs51_write(i[7:0], 8'h5A + i);
        end

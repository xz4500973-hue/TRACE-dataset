...
(* keep_hierarchy = "yes" *) module ClaiRISC_core (
        input clk,
        input rst,	 
    ...
    always @(*) begin
        // if-else (was case)
            if(w_alu_op_r==`ALU_ADD)   {addercout,  w_alu_res}  = w_alu_in_a + w_alu_in_b;
            else if(w_alu_op_r==`ALU_SUB)  {addercout,  w_alu_res}  = w_alu_in_b - w_alu_in_a;
            else if(w_alu_op_r==`ALU_ROR)  {addercout,  w_alu_res}  = {w_alu_in_b[0], w_c_2alu, w_alu_in_b[7:1]};
            else if(w_alu_op_r==`ALU_ROL)  {addercout,  w_alu_res}  = {w_alu_in_b[7],w_alu_in_b[6:0], w_c_2alu};
            else if(w_alu_op_r==`ALU_OR)   {addercout,  w_alu_res}  = {1'bx, w_alu_in_a | w_alu_in_b};
            else if(w_alu_op_r==`ALU_XOR)  {addercout,  w_alu_res}  = {1'bx, w_alu_in_a ^ w_alu_in_b};
            else if(w_alu_op_r==`ALU_COM)  {addercout,  w_alu_res}  = {1'bx, ~w_alu_in_b};
            else if(w_alu_op_r==`ALU_SWAP) {addercout,  w_alu_res}  = {1'bx, w_alu_in_b[3:0], w_alu_in_b[7:4]};
            else if(w_alu_op_r==`ALU_AND || w_alu_op_r==`ALU_BTFSC || w_alu_op_r==`ALU_BTFSS)//:  {addercout,  y}  = {1'bx, a & b};
            //:  {addercout,  y}  = {1'bx, a & b };
             {addercout,  w_alu_res}  = {1'bx, w_alu_in_a & w_alu_in_b };
            else if(w_alu_op_r==`ALU_DEC)   {addercout,  w_alu_res}  = {1'bx, w_alu_in_b - 1};
            else if(w_alu_op_r==`ALU_INC)   {addercout,  w_alu_res}  = {1'bx, 1 + w_alu_in_b};
            else if(w_alu_op_r==`ALU_PA)   {addercout,  w_alu_res}  = {1'bx, w_alu_in_a};
            else if(w_alu_op_r==`ALU_PB)   {addercout,  w_alu_res}  = {1'bx, w_alu_in_b};
            else if(w_alu_op_r==`ALU_BSF)  {addercout,  w_alu_res}  = {1'Bx,w_alu_in_a | w_alu_in_b};
            else if(w_alu_op_r==`ALU_BCF)  {addercout,  w_alu_res}  = {1'bx,~w_alu_in_a & w_alu_in_b};
            else {addercout, w_alu_res}  = {1'bx, 8'h00};
    end

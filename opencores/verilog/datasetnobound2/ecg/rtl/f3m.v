...
(* keep_hierarchy = "yes" *) module f3m_mux3(v1, l1, v2, l2, v3, l3, out);
...
(* keep_hierarchy = "yes" *) module f3m_mux6(v0, v1, v2, v3, v4, v5, l0, l1, l2, l3, l4, l5, out);
...
(* keep_hierarchy = "yes" *) module f3m_add(A, B, C);
    input [`WIDTH : 0] A, B;
    output [`WIDTH : 0] C;
    genvar i;
    generate
...
(* keep_hierarchy = "yes" *) module f3m_add3(a0, a1, a2, c);
...
(* keep_hierarchy = "yes" *) module f3m_add4(a0, a1, a2, a3, c);
...
(* keep_hierarchy = "yes" *) module f3m_neg(a, c);
    input [`WIDTH:0] a;
    output [`WIDTH:0] c;
    genvar i;
    generate
...
(* keep_hierarchy = "yes" *) module f3m_sub(A, B, C);
    input [`WIDTH : 0] A, B;
    output [`WIDTH : 0] C;
    genvar i;
    generate
...
// f3m_mult: C = A * B, in field GF(3^M) -- refactored: 2-segment explicit FSM
(* keep_hierarchy = "yes" *) module f3m_mult(clk, reset, A, B, C, done);
    input [`WIDTH : 0] A, B;
    input clk;
    input reset;
    ...
    localparam S_COMPUTE = 2'b00;
    localparam S_DONE    = 2'b01;
    ...
    always @(posedge clk)
        ...
        else if (state == S_COMPUTE && next_state == S_DONE)
            done <= 1;
    ...
    always @(posedge clk) begin
        if (reset) begin
            ...
            x <= z3[`WIDTH:0]; y <= z4; z <= z2;
        end
    end
endmodule

// c0 == a0*b0; c1 == a1*b1; c2 == a2*b2; all in GF(3^M) -- refactored: 2-segment explicit FSM
(* keep_hierarchy = "yes" *) module f3m_mult3(clk, reset, 
                 a0, b0, c0,
                 a1, b1, c1, 
                 a2, b2, c2, 
                 done);
    ...
    output reg done;
    reg mult_reset;
    wire mult_done, delay3;
    wire [`WIDTH:0] in1, in2, o;
    
    localparam S_M0 = 3'b000;
    localparam S_M1 = 3'b001;
    localparam S_M2 = 3'b010;
    localparam S_DONE = 3'b011;
    localparam S_IDLE = 3'b100;
    ...
    always @(*) begin
        next_state = state;
        case (state)
            S_IDLE: next_state = S_M0;
            S_M0: if (mult_done) next_state = S_M1;
            S_M1: if (mult_done) next_state = S_M2;
            S_M2: if (mult_done) next_state = S_DONE;
    ...
    always @(posedge clk)
        if (reset)
            ...
            mult_reset <= 1;
        else if (state == S_M0 && next_state == S_M1)
            mult_reset <= 1;
        else if (state == S_M1 && next_state == S_M2)
            mult_reset <= 1;
        else
            mult_reset <= 0;
...
/* out == in^3 mod p(x) */
/* p(x) == x^97 + x^12 + 2 */
(* keep_hierarchy = "yes" *) module f3m_cubic(input wire [193:0] in, output wire [193:0] out);
...
/* p(x) == x^97 + x^12 + 2 */
(* keep_hierarchy = "yes" *) module f3m_nine(clk, in, out);
    input clk;
    input [`WIDTH:0] in;
...
// inversion in GF(3^m). C = A^(-1) -- refactored: 2-segment explicit FSM
(* keep_hierarchy = "yes" *) module f3m_inv(clk, reset, A, C, done);
    input [`WIDTH:0] A;
    input clk;
    input reset;
    output reg [`WIDTH:0] C;
    ...
    always @(*) begin
        next_state = state;
        case (state)
            S_IDLE: next_state = S_COMPUTE;
            S_COMPUTE: begin
                if (U2[`WIDTH] || (S == `PX && R == A && U == 1))
                    next_state = S_COMPUTE;
                else if (R[`MOST] == 2'b0 || d[0] == 1'b0)
                    next_state = S_COMPUTE;
                else
                    next_state = S_COMPUTE;
            end
    ...
    always @(posedge clk)
        if (state == S_COMPUTE && next_state == S_DONE)
            C <= U2[`WIDTH:0];
...
// out = S - q*R
(* keep_hierarchy = "yes" *) module func1(S, R, q, out);
    input [`WIDTH+2:0] S, R;
    input [1:0] q;
...
// out = x*A
(* keep_hierarchy = "yes" *) module func2(A, out);
    input [`WIDTH+2:0] A;
    output [`WIDTH+2:0] out;
...
// C = (x*B mod p(x))
(* keep_hierarchy = "yes" *) module func3(B, C);
    input [`WIDTH+2:0] B;
    output [`WIDTH+2:0] C;
    wire [`WIDTH+2:0] A;
...
(* keep_hierarchy = "yes" *) module func4(A, aa, C);
    input [`WIDTH+2:0] A;
    input [1:0] aa;
    output [`WIDTH+2:0] C;
    genvar i;
...
(* keep_hierarchy = "yes" *) module func5(A, C);
    input [`WIDTH+2:0] A;
    output [`WIDTH+2:0] C;
    assign C[195:194] = 0;
...
/* $C=A*(x^3) mod p(x)$ */
/* p(x) == x^97 + x^12 + 2 */
(* keep_hierarchy = "yes" *) module func7(input wire [193:0] in, output wire [193:0] out);
...
(* keep_hierarchy = "yes" *) module func8 (a, b, c);
input [193:0] a;
input [5:0] b;
output [193:0] c;

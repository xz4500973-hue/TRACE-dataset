...
always @(posedge sys_clk) begin
	if(sys_rst) begin
		...
		if(csr_selected) begin
			if(csr_we) begin
				if(csr_a[3:0] == 4'd0) begin
						tx_rst <= csr_di[1];
						rx_rst <= csr_di[0];
					end
					else if(csr_a[3:0] == 4'd1) begin
						phy_mii_clk <= csr_di[3];
						mii_data_oe <= csr_di[2];
						mii_data_do <= csr_di[0];
					end
					else if(csr_a[3:0] == 4'd2) begin
						slot0_state <= csr_di[1:0];
						slot0_count <= 11'd0;
					end
					else if(csr_a[3:0] == 4'd3) slot0_adr <= csr_di[31:2];
					else if(csr_a[3:0] == 4'd5) begin
						slot1_state <= csr_di[1:0];
						slot1_count <= 11'd0;
					end
					else if(csr_a[3:0] == 4'd6) slot1_adr <= csr_di[31:2];
					else if(csr_a[3:0] == 4'd8) begin
						slot2_state <= csr_di[1:0];
						slot2_count <= 11'd0;
					end
					else if(csr_a[3:0] == 4'd9) slot2_adr <= csr_di[31:2];
					else if(csr_a[3:0] == 4'd11) begin
						slot3_state <= csr_di[1:0];
						slot3_count <= 11'd0;
					end
					else if(csr_a[3:0] == 4'd12) slot3_adr <= csr_di[31:2];
					else if(csr_a[3:0] == 4'd14) tx_adr <= csr_di[31:2];
					else if(csr_a[3:0] == 4'd15) begin

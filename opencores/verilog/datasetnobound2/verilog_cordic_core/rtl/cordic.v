/* cordic.v - dataset2: iterative shifter, case-based rotator */
...
integer j;
always@* begin Q=D; for(j=0;j<i;j=j+1) Q=(Q>>1)|(D[`XY_BITS]<<`XY_BITS); end
endmodule
...
// Combined always block with if-else
always@(posedge clk)
  if(rst) begin x_1<=0; y_1<=0; z_1<=0; end
  else begin
    if(z_i<0) begin
      x_1<=x_i+y_i_shifted; y_1<=y_i-x_i_shifted; z_1<=z_i+tangle;
    end else begin
      x_1<=x_i-y_i_shifted; y_1<=y_i+x_i_shifted; z_1<=z_i-tangle;
    end
  end
...
wire signed[`XY_BITS:0] x[`ITERATIONS-1:0],y[`ITERATIONS-1:0];
wire signed[`THETA_BITS:0] z[`ITERATIONS-1:0];
assign x[0]=x_i; assign y[0]=y_i; assign z[0]=theta_i;
assign x_o=x[`ITERATIONS-1]; assign y_o=y[`ITERATIONS-1]; assign theta_o=z[`ITERATIONS-1];

genvar i; generate for(i=0;i<`ITERATIONS-1;i=i+1) begin:stg

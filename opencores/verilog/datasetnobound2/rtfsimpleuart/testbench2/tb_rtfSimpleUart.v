...
module tb_rtfSimpleUart;
    ...
    task uart_send_byte; input [7:0] byte3; integer i;
        begin rxd=1'b0; repeat(BIT_CLKS) @(posedge clk);
        for(i=0;i<8;i=i+1) begin rxd=byte3[i]; repeat(BIT_CLKS) @(posedge clk); end
    ...
    initial begin
        ...
        uart_send_byte(8'h55); while(!data_present) @(posedge clk);
        uart_send_byte(8'hAA); while(!data_present) @(posedge clk);
        uart_send_byte(8'h0F); while(!data_present) @(posedge clk);
        uart_send_byte(8'hF0); while(!data_present) @(posedge clk);
        #30000;

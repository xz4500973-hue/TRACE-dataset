...
module tb_P16C5x_v;
	...
	// Memory/RAM test: fill RAM area 0x10-0x17 with incrementing values, then read back
	always @(posedge Clk or posedge POR)
	begin
		if(POR) ROM <= 12'b1010_0000_0000;
		else case(PC[11:0])
			12'h000: ROM <= #1 12'b1100_0001_0000; // MOVLW 0x10
			12'h001: ROM <= #1 12'b0000_0010_0100; // MOVWF 0x04 (FSR=0x10)
			12'h002: ROM <= #1 12'b1100_0000_1000; // MOVLW 8
			12'h003: ROM <= #1 12'b0000_0010_1000; // MOVWF 0x08 (counter=8)
			12'h004: ROM <= #1 12'b1100_0001_0001; // MOVLW 0x11
			12'h005: ROM <= #1 12'b0010_0000_0000; // MOVF 0x00,0 (read INDF into W)
			12'h006: ROM <= #1 12'b0010_1010_0100; // INCF 0x04,1  (FSR++)
			12'h007: ROM <= #1 12'b0010_1110_1000; // DECFSZ 0x08,1
			12'h008: ROM <= #1 12'b1010_0000_0101; // GOTO 0x005
			12'h009: ROM <= #1 12'b1010_0000_0000; // GOTO 0x000

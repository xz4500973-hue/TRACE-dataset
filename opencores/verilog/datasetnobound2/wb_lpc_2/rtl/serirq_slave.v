//////////////////////////////////////////////////////////////////////
////  serirq_slave.v - SERIRQ Slave (dataset2)                     ////
////  Refactored: stop detection merged into main FSM               ////
//////////////////////////////////////////////////////////////////////

`timescale 1 ns / 1 ns
`include "serirq_defines.v"

(* keep_hierarchy = "yes" *) module serirq_slave(clk_i, nrst_i, 
                    irq_i,
                    serirq_o, serirq_i, serirq_oe
);
    input             clk_i;
    input             nrst_i;
    output reg        serirq_o;
    input             serirq_i;
    output reg        serirq_oe;
    input      [31:0] irq_i;
    reg        [31:0] current_irq;
    reg        [12:0] state;
    reg         [4:0] irq_cnt;
    reg found_stop;
    reg found_start;
    reg serirq_mode;
    reg [3:0] stop_clk_cnt;

    wire irq_changed = (serirq_mode & (current_irq != irq_i));
    
    // Merged sequential block (stop detection integrated)
    always @(posedge clk_i or negedge nrst_i)
        if(~nrst_i) begin
            state     <= `SERIRQ_ST_IDLE;
            serirq_oe <= 1'b0;
            serirq_o  <= 4'b1;
            irq_cnt   <= 5'h00;
            current_irq <= irq_i;
            found_stop  <= 1'b0;
            found_start <= 1'b0;
            serirq_mode <= `SERIRQ_MODE_CONTINUOUS;
            stop_clk_cnt <= 4'h0;
        end
        else begin
            // Stop detection (inline)
            if(serirq_i == 1'b0) begin
                stop_clk_cnt <= stop_clk_cnt + 1;
            end else begin
                case (stop_clk_cnt)
                    4'h2: begin found_stop <= 1'b1; found_start <= 1'b0; serirq_mode <= `SERIRQ_MODE_QUIET; end
                    4'h3: begin found_stop <= 1'b1; found_start <= 1'b0; serirq_mode <= `SERIRQ_MODE_CONTINUOUS; end
                    4'h4,4'h6,4'h8: begin found_stop <= 1'b0; found_start <= 1'b1; end
                    default: begin found_stop <= 1'b0; found_start <= 1'b0; end
                endcase
                stop_clk_cnt <= 4'h0;
            end

            // Main FSM (if-else chain)
            if(state == `SERIRQ_ST_IDLE) begin
                serirq_oe <= 1'b0;
                irq_cnt   <= 5'h00;
                serirq_o  <= 1'b1;
                if(found_start == 1'b1) begin
                    current_irq <= irq_i;
                    serirq_oe <= irq_i[irq_cnt] ? 1'b0 : 1'b1;
                    serirq_o  <= irq_i[irq_cnt] ? 1'b1 : 1'b0;
                    state <= `SERIRQ_ST_IRQ_R;
                end else if(irq_changed) begin
                    current_irq <= irq_i;
                    serirq_o <= 1'b0;
                    serirq_oe <= 1'b1;
                    state <= `SERIRQ_ST_IDLE;
                end else state <= `SERIRQ_ST_IDLE;
            end
            else if(state == `SERIRQ_ST_IRQ) begin
                serirq_oe <= irq_i[irq_cnt] ? 1'b0 : 1'b1;
                serirq_o  <= irq_i[irq_cnt] ? 1'b1 : 1'b0;
                state <= found_stop ? `SERIRQ_ST_IDLE : `SERIRQ_ST_IRQ_R;
            end
            else if(state == `SERIRQ_ST_IRQ_R) begin
                serirq_o <= 1'b1;
                state <= found_stop ? `SERIRQ_ST_IDLE : `SERIRQ_ST_IRQ_T;
            end
            else if(state == `SERIRQ_ST_IRQ_T) begin
                serirq_oe <= 1'b0;
                if(irq_cnt == 5'h1f) state <= `SERIRQ_ST_WAIT_STOP;
                else begin irq_cnt <= irq_cnt + 1; state <= found_stop ? `SERIRQ_ST_IDLE : `SERIRQ_ST_IRQ; end
            end
            else if(state == `SERIRQ_ST_WAIT_STOP) begin
                state <= found_stop ? `SERIRQ_ST_IDLE : `SERIRQ_ST_WAIT_STOP;
            end
        end
endmodule
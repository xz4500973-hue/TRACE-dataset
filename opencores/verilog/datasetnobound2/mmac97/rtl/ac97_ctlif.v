...
(* keep_hierarchy = "yes" *) module ac97_ctlif #(
...
always @(posedge sys_clk) begin
	if(sys_rst) begin
		...
		if(csr_selected) begin
			if(csr_we) begin
				if(csr_a[3:0] == 4'b0000) begin
						request_en <= csr_di[0];
						request_write <= csr_di[1];
					end
					else if(csr_a[3:0] == 4'b0001) request_addr <= csr_di[6:0];
					else if(csr_a[3:0] == 4'b0010) request_data <= csr_di[15:0];
					else if(csr_a[3:0] == 4'b0100) dmar_en <= csr_di[0];
					else if(csr_a[3:0] == 4'b0101) dmar_addr <= csr_di[31:2];
					else if(csr_a[3:0] == 4'b0110) dmar_remaining <= csr_di[17:2];
					else if(csr_a[3:0] == 4'b1000) dmaw_en <= csr_di[0];
					else if(csr_a[3:0] == 4'b1001) dmaw_addr <= csr_di[31:2];
					else if(csr_a[3:0] == 4'b1010) dmaw_remaining <= csr_di[17:2];
			end
			case(csr_a[3:0])

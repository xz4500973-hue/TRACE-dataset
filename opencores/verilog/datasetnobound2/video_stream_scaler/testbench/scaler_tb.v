/*------------------------------------------------------------------------------
	Video Stream Scaler testbench (dataset2: narrower scaling factors)
-------------------------------------------------------------------------------*/
...
initial begin
	...
	rfile = $fopen("input640x512RGB.raw", "rb");
	dIn=0; dInValid=0; start=0;
	#60 start=1; #10 start=0;
	#20 r=$fread(readMem,rfile); dIn=readMem[0];
	while(!$feof(rfile)) begin
		dInValid=1; #10;
		if(nextDin) begin r=$fread(readMem,rfile); dIn=readMem[0]; end
	end
	$fclose(rfile);
end

initial begin nextDout=0; #50000; forever begin nextDout=1; #((OUTPUT_X_RES+1)*20); nextDout=0; #((OUTPUT_X_RES+1)*100); end end
...
initial begin
	...
	while(dOutCount<(OUTPUT_X_RES+1)*(OUTPUT_Y_RES+1)) begin
		#10;
		if(dOutValid==1) dOutCount=dOutCount+1;
	end

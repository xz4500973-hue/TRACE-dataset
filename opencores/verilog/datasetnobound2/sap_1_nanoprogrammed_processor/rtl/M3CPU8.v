`timescale 1ns / 1ps
// M3CPU8 (dataset2: if-else refactored submodules)
...
// Dataset2: MICRO_PROG_COUNTER with nxt_ logic
...
wire [4:0] nxt_PRE;
assign nxt_PRE = LOAD_MICRO ? AR_ROM_IN : (INC_MICRO ? (PRE_OUT_r + 1'b1) : (CLEAR_MICRO ? 5'h00 : PRE_OUT_r));
always@(posedge clk or posedge rst) begin if(rst) PRE_OUT_r<=5'h00; else PRE_OUT_r<=nxt_PRE; end
endmodule
...
// Dataset2: ALU with case instead of ternary chains
...
always@(ALU_A or ALU_B or SU or AD) begin
    if (SU) ALU_OUT_r = ALU_A - ALU_B;
    else if (AD) ALU_OUT_r = ALU_A + ALU_B;
    else ALU_OUT_r = 9'h000;
end
endmodule
